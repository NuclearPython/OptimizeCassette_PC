...
def main_multi(gh):
    ...
    #ADDED _multi to a bunch of things
    #made it return the last population as well
    ...
    num_objective_functions = gh.numObjectiveFunctions

    # Check for objective function(s)
    #assert hasattr(gh.objective.func, '__call__'), ('Invalid function ' 'handle provided.')
    ...
    if num_objective_functions > 1:
        populationSize = gh.population
        #parent_fitness = np.ones((num_objective_functions, initNum)) #this shape may be causeing problems in rejections bounds 
        parent_fitness = np.ones(( initNum, num_objective_functions))
        initNum = min(initNum, len(initParams))
        for p in range(0, initNum, 1):
            #pop.append(Parent_multi(fitness=1E99*parent_fitness[:,p], variables=initParams[p]))
            pop.append(Parent_multi(fitness=1E99*parent_fitness[p,:], variables=initParams[p]))
    else:
        initNum = min(initNum, len(initParams))
        for p in range(0, initNum, 1):
            pop.append(Parent_multi(fitness=1E99, variables=initParams[p]))

    # Calculate initial fitness values and trim population to gh.population
    (pop, changes, timeline) = gh.population_update_multi(pop,
    ...
    while converge == False:
        ...
        (pop, changes, timeline) = gh.population_update_multi(pop, children,
                                                        timeline=timeline,
                                                        adoptedParents=ind,
                                                        mhFrac=0.2,
                                                        randomParents=True)
