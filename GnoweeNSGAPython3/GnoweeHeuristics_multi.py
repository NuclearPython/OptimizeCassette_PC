...
class GnoweeHeuristics_multi(ProblemParameters_multi):
    ...
    def __str__(self):
        ...
        header += ["{}".format(ProblemParameters_multi.__str__(self))]
        return "\n".join(header)+"\n"
    ...
    def population_update_multi(self, parents, children, timeline=None,
        ...
        @return <em> list of history objects: </em> If an initial timeline was
            ...
            evaluations.
        """
        #takes in children and produces an evaluated set of parents

        #Test input values for consistency
        #assert hasattr(self.objective.func, '__call__'), ('Invalid \tion ' 'handle.')
        ...
        if numFunctions == 1:
            ...
            #create a list of tuples containing the variable ranges
            var_range = []
            for k in range(0, num_cont_int_bin_variables):
                var_range += (self.lb[k], self.ub[k])
            #create the problem variable for use with outside method
            ...
            #create population variable formulation in order to use outside method
            ...
            for p in range(0, num_children): #
                #create individual from a child
                test_individual = problem.generate_individual()
                test_individual.features[:] = cp.copy(children[p])
                #get it fitness
                problem.calculate_objectives(test_individual)
                #add it to the population
                population.append(test_individual)
            #add the parents to the population
            for t in range(0, num_parents):
                #create individual from a parent
                test_individual = problem.generate_individual()
                test_individual.features[:] = cp.copy(children[p])
                #get it fitness
                problem.calculate_objectives(test_individual)
            ...
            """
            newish_population = []
            #need to add in the adopted parents

            GnoweeParentObjectList = parents
            GnoweeChildList = children
            newish_population = NSGAmethods.runNSGA_generation(GnoweeParentObjectList,GnoweeChildList, self) #self is used here because self inherits from Problem Parameters
            newish_population.population.sort(key=lambda x: x.rank) #sort the outgoing population in a way that approximates what Gnowee is expecting
            #convert the new population into the Gnowee expected 'parents'
            ...
            #calculate feval (the number of function evaluations)
            feval = num_children
        ...
        if timeline != None:
            if numFunctions == 1:
                ...
                #in the future I need to do something  better than just using the first fitness...
                if len(timeline) < 2:
                    timeline.append(Event(1, feval, parents[0].fitness[0],
                                          parents[0].variables))
                elif parents[0].fitness[0] < timeline[-1].fitness[0] \
                      and abs((timeline[-1].fitness-parents[0].fitness[0]) \
                              /parents[0].fitness[0]) > self.convTol:
                    timeline.append(Event(timeline[-1].generation,
                                          timeline[-1].evaluations+feval,
                                          parents[0].fitness[0], parents[0].variables))
                else:
                    timeline[-1].generation += genUpdate
                    timeline[-1].evaluations += feval
